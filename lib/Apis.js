...
class Apis extends CappuccinosBase_1.CappuccinosBase {
    constructor(env, options, logger, config, awsConfig) {
        super(env, options, logger, config, awsConfig, path_1.default.resolve('./build/apis'));
    ...
    async makeDocumentAll() {
        await Promise.all(this.projectConfig.apis.map(api => this.makeDocument(api)));
    }
    async makeDocument(config) {
        ...
        const out = child_process_1.execSync(`spectacle --target-dir ${this.buidDir}/${apiName} ${path}`);
        this.logger.debug(out.toString());
        this.logger.info(`  # API document created        ${colorette_1.blue('api=')}${apiName}`);
    }
    async mergeSwaggerFile(config) {
        const apiName = config.name;
        ...
        glob_1.sync(`apis/${apiName}/paths/*.yaml`).map(path => {
            const paths = utils.loadYaml(path);
            Object.keys(paths).map(key => {
                if (swg.paths[key] === undefined) {
                    if (this.strictPaths)
    ...
    async uploadSwaggerFiles() {
        await Promise.all(this.projectConfig.apis.map(api => this.uploadSwaggerFile(api)));
    }
    async uploadSwaggerFile(config) {
        const apiName = config.name;
        const yaml = await this.mergeSwaggerFile(config);
