...
class Logs extends CappuccinosBase_1.CappuccinosBase {
    ...
    async init() {
        var _a;
        for (let token = undefined, count = 0;; count++) {
            const params = {
            ...
            (_a = resp.logGroups) === null || _a === void 0 ? void 0 : _a.forEach(lg => {
                this.logGroups.push(lg);
            });
            if (token === undefined)
                break;
            if (count % 5 === 4)
                await utils.sleep(1000); // API Rate Limit
        }
    }
    async updateAll() {
    ...
    async update(functionPath) {
        ...
        if ((logGroup === null || logGroup === void 0 ? void 0 : logGroup.retentionInDays) === retentionInDays)
            return;
    ...
    async subscribeAll() {
        ...
        for (let i = 0, len = functions.length; i < len; i++) {
            await this.subscribe(utils.toFunctionPath(functions[i]));
