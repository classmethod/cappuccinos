...
class WebSocketApis extends CappuccinosBase_1.CappuccinosBase {
    ...
    async getIntegrationFunctions(apiId) {
        ...
        const functions = (_a = result.Items) === null || _a === void 0 ? void 0 : _a.filter(item => item.IntegrationType === 'AWS_PROXY').map(item => {
            if (item.IntegrationUri === undefined)
                return undefined;
            const m = item.IntegrationUri.match(/\:function\:([a-z0-9_]+)[\:\/]/);
            return m ? m[1] : undefined;
        }).filter(utils.notUndefined).filter((x, i, self) => self.indexOf(x) === i); // unique;
