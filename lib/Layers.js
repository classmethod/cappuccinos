...
class Layers {
    ...
    async buildAll() {
        await Promise.all(this.projectConfig.layers.filter(layer => layer.build !== undefined).map(layer => this.build(layer.name)));
    }
    ...
    async deployAll() {
        await Promise.all(this.projectConfig.layers.filter(layer => layer.build !== undefined).map(layer => this.deploy(layer.name)));
    }
    async deploy(layerName) {
        const file = await this.build(layerName);
        if (file === undefined)
            throw new Error(`Cannot deploy Layer: ${layerName}`);
