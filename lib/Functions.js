...
class Functions {
    ...
    async addPermission(functionName, alias, permission) {
        const params = {
            FunctionName: functionName,
            Qualifier: alias,
    ...
    async updatePermission(functionName, alias, permission, policy) {
    ...
    isNeedUpdate(permission, policy) {
        ...
        if (policy.Action !== permission.action)
            return true;
        if (policy.Condition.StringEquals && policy.Condition.StringEquals['AWS:SourceAccount'] !== permission.source_account)
            return true;
        if (policy.Condition.ArnLike && policy.Condition.ArnLike['AWS:SourceArn'] !== permission.source_arn)
            return true;
        return false;
    }
    async removePermission(functionName, alias, statementId) {
        const params = {
            FunctionName: functionName,
            Qualifier: alias,
