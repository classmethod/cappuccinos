...
exports.loadYaml = async (path, transformer) => {
    try {
...
exports.loadProjectConfig = async (env, awsConfig) => {
    ...
    const config = await exports.loadYaml(`./conf/project.yaml`, transformer);
    const envConf = await exports.loadYaml(`./conf/${env}/project.yaml`, transformer);
    if (envConf) {
        Object.keys(envConf).map(key => {
            config[key] = envConf[key];
        });
    }
    const functionsEnvConf = await exports.loadYaml(`./conf/${env}/functions.yaml`, transformer);
    if (functionsEnvConf) {
